...
void insert_queue(struct queue2 *,BitNode *);
BitNode * delete_queue(struct queue2 *);
...
{
    qu->T =(BiTree *)malloc(sizeof(BiTree) * size);
    if(NULL == qu->T)
    {
...
void insert_queue(struct queue2 * qu,BitNode * q)
{
    if(is_full(qu))
    {
...
BitNode * delete_queue(struct queue2 * qu)
{
    BitNode  * temp;
    if(is_empty(qu))
    {
